...
export async function GET(request: NextRequest) {
  ...
  const { data: instructor } = await supabase.from("instructors").select("full_name").eq("id", instructorId).single();
  ...
  const { data: lessons } = await supabase
    .from("lessons")
    .select("id,lesson_date,start_time,status,location:locations!lessons_location_id_fkey(name,city)")
  ...
  const rows = lessons.map((lesson: any, i: number) => {
    ...
    return `<tr style="${i%2===1?"background:#f9f9fb":""}">
      ...
      <td>${(lesson.location as any)?.city ?? "—"}</td>
  ...
  const instrConf   = [...sigMap.values()].filter((s:any) => s.signer_role==="instructor").length;

  const html = `<!DOCTYPE html>
<html dir="rtl" lang="he">
<head>
<meta charset="UTF-8">
<title>דוח חודשי — ${instructor.full_name}</title>
...
<div class="sub">${instructor.full_name}<br>${MONTHS_HEBREW[month-1]} ${year}</div>
<hr>
<table>
<thead><tr><th>תאריך</th><th>יום</th><th>שעה</th><th>מיקום</th><th>עיר</th><th>סטטוס</th><th>אישור</th></tr></thead>
<tbody>${rows}</tbody>
</table>
<div class="summary">סה"כ שיעורים: ${lessons.length} | הושלמו: ${completed} | בוטלו: ${cancelled} | אישור גננת: ${teacherConf} | אישור מדריכה: ${instrConf}</div>
