...
export async function GET(request: NextRequest) {
  ...
  const [recurringLessonsRes, oneOffLessonsRes] = await Promise.all([
    recurringIds.length > 0
      ? supabase.from("lessons").select(lessonSelect).in("recurring_item_id", recurringIds)
          .gte("lesson_date", startDate).lte("lesson_date", endDate)
      : Promise.resolve({ data: [], error: null }),
    supabase.from("lessons").select(lessonSelect).eq("client_name", clientName)
      .gte("lesson_date", startDate).lte("lesson_date", endDate),
  ]);
  ...
  const lessons = [...byId.values()].sort((a: any, b: any) =>
    a.lesson_date === b.lesson_date ? a.start_time.localeCompare(b.start_time) : a.lesson_date.localeCompare(b.lesson_date)
  );
