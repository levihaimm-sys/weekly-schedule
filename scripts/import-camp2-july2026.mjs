...
const csvPath = "C:/Users/levih/Documents/weekly-schedule/מחזור 2 קייטנת יולי 2026 לטעינה.csv";
...
for (const row of rows) {
  ...
  const gardenName = row["שם הצהרון"].trim();
  ...
  const instructorName = row["מדריכה"].trim();
  ...
  const notesParts = [];
  if (row["חוג/הפעלה"]) notesParts.push(row["חוג/הפעלה"]);
  if (row["קבוצה"]) notesParts.push(`קבוצה: ${row["קבוצה"]}`);
  if (row["גיל"]) notesParts.push(`גיל/כיתה: ${row["גיל"]}`);
  if (row["איש קשר"]) notesParts.push(`איש קשר: ${row["איש קשר"]}`);
...
fs.writeFileSync(
  "C:/Users/levih/Documents/weekly-schedule/scripts/.camp2-dryrun.json",
  JSON.stringify({ prepared, locationsToCreate: [...locationsToCreate.entries()], problems, duplicatesInFile }, null, 2),
  "utf8"
);
console.log("\nDry-run data written to scripts/.camp2-dryrun.json");
